...
def search(results,encodedTitle,title,searchTitle,siteNum,lang,searchByDateActor,searchDate,searchSiteID):
    ...
    #
    #     url = PAsearchSites.getSearchBaseURL(searchSiteID) + scenePageBase + searchTitle.lower().replace(" ","_").replace("'","_")
    ...
    #     curID = scenePageBase + searchTitle.lower().replace(" ","_").replace("'","_")
    #     curID = curID.replace("/","?")
    #     Log("curID: " + curID)
    #
...
def update(metadata,siteID,movieGenres,movieActors):
    temp = str(metadata.id).split("|")[0].replace("_","/").replace("-","_").replace('_','-',-1)
