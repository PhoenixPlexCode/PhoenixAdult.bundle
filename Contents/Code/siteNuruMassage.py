...
def search(results,encodedTitle,title,searchTitle,siteNum,lang,searchByDateActor,searchDate, searchAll, searchSiteID):
    ...
    for searchResult in searchResults.xpath('//div[@class="tlcDetails"]'):
        ...
        results.Append(MetadataSearchResult(id = curID + "|" + str(siteNum), name = titleNoFormatting + " [Nuru Massage] " + releaseDate, score = score, lang = lang))
    return results
...
def update(metadata,siteID,movieGenres,movieActors):
    ...
    # Summary
    metadata.summary = detailsPageElements.xpath('//meta[@name="twitter:description"]')[0].get('content').strip().replace("</br>","\n")
    #paragraph = paragraph.replace('&13;', '').strip(' \t\n\r"').replace('\n','').replace('  ','') + "\n\n"
    metadata.collections.clear()
    tagline = "Nuru Massage"
        ...
        if not posterAlreadyExists(posterUrl,metadata):
            ...
            try:
                ...
                if(width > 1):
                    ...
                    metadata.posters[posterUrl] = Proxy.Preview(HTTP.Request(posterUrl, headers={'Referer': photoPageUrl}).content, sort_order = i)
                if(width > 100):
                    # Item is an art item
                    metadata.art[posterUrl] = Proxy.Preview(HTTP.Request(posterUrl, headers={'Referer': photoPageUrl}).content, sort_order = i+1)
                i = i + 1
