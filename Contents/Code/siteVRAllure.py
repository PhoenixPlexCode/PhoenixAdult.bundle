...
def search(results, lang, siteNum, searchData):
    ...
    results.Append(MetadataSearchResult(id='%s|%d' % (curID, siteNum), name='%s [VRAllure] %s' % (titleNoFormatting, releaseDate), score=score, lang=lang))
...
def update(metadata, lang, siteNum, movieGenres, movieActors, art):
    ...
    # Summary
    metadata.summary = detailsPageElements.xpath('//p[@class = "desc"]//span')[0].text_content().strip()

    # Studio
    metadata.studio = 'VRAllure'
    ...
    # Release Date
    date = parse(detailsPageElements.xpath('//p[@class = "publish-date"]')[0].text_content().strip()).strftime('%Y-%m-%d')
    if date:
    ...
    # Genres
    genres = detailsPageElements.xpath('//a[contains(@class, "label label-tag")]')
    ...
    # Actor(s)
    for actorLink in detailsPageElements.xpath('//p[@class="model-name"]//a[contains(@href, "/models/")]'):
