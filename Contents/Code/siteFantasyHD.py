...
def search(results,encodedTitle,title,searchTitle,siteNum,lang,searchByDateActor,searchDate,searchAll,searchSiteID):
    url = 'http://fantasy-hd.com/video/' + searchTitle.lower().replace(" ","-")
    ...
    titleNoFormatting = girlName + " - " + titleNoFormatting + " [Fantasy-HD, " + releasedDate +"]"
    score = 100
...
def update(metadata,siteID,movieGenres,movieActors):
    ...
    if len(actors) > 0:
        for actorLink in actors:
            actorPageURL = 'http://fantasy-hd.com' + actorLink.get("href")
